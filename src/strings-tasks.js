...
function isPalindrome(str) {
  const cleannedStr = str.replace(/[^a-z0-9]/gi, '').toLowerCase();
  const lastIndex = cleannedStr.length - 1;
  // eslint-disable-next-line no-plusplus
  for (let i = 0; i < cleannedStr.length / 2; i++) {
    if (cleannedStr[i] !== cleannedStr[lastIndex - i]) {
      return false;
    }
  }
  return true;
...
function extractNameFromTemplate(value) {
  const cleanedStr = value
    ...
    .trim()
    .split(/\s+/);
  return `${cleanedStr[1]} ${cleanedStr[2]}`;
}
